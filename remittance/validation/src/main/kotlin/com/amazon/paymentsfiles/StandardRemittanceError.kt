...
enum class StandardRemittanceError(val error: FieldError) {
    DepositAccountNumber(
        FieldError("Deposit Account Number must be numeric and cannot exceed 32 characters in length")
    ),
    FileFormatVersion(FieldError("File Format Version was expected to be $REMITTANCE_FILE_VERSION")),
    IncorrectNumberOfRemittanceRecords(
        FieldError(
            "Number of Remittance Records in trailer does not match the file",
    ...
    IncorrectNumberOfRecords(
        FieldError(
            "Number of Records in deposit trailer is does not match the deposit",
    ...
    DifferingDepositDates(
        FieldError("The Deposit Date in the Deposit Trailer must match the corresponding Deposit Header")
    ),
    FXRate(FieldError("Transaction FX Rate must be specified if FX Rate is left blank in the Deposit Header"));

    companion object {
        fun notStandardMonetary(fieldName: String) = FieldError(
            "$fieldName not a proper numeric decimal value",
